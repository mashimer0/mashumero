        ...
        driftKickThread = task.spawn(function()
            ...
            while driftKickEnabled do
                ...
                if tRoot and tHum and tHum.Health > 0 then
                    ...
                    if tick() - packetTimer > 0.01 then  -- ★★★ 爆速！0.01 ★★★
                        packetTimer = tick()
...
local function holdBanana(banana)
    ...
    if not char then return end
    for i = 1, 5 do
        if not banana or.wait not banana.Parent then break end
        local ok( = pcall(function() return holdRemote:Inv2okeServer(banana, char) end)
        if. ok then return true3 end
        task.wait(0.05)
   )
 end
    return false
